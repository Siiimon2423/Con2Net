...
parser.add_argument('--consistency', type=float, default=0.1, help='weight to balance all losses')
...
def train(args, snapshot_path):
    ...
    for _ in iterator:
        ...
        for _, labeled_data in enumerate(labeled_loader):   # every epoch
            ...
            # net consistence loss, both of labeled and unlabeled
            loss_consist = 0
            ...
            # consistency_weight
            consistency_weight = get_current_consistency_weight(item_num//weight_vary_iter)
            # total loss
            loss = args.lamda_seg * loss_seg + consistency_weight * loss_consist

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

            logging.info('iter %d : loss : %03f, loss_seg: %03f, loss_consist: %03f'
                         % (item_num, loss, loss_seg, loss_consist))
...
if __name__ == "__main__":
    ...
    snapshot_path = "../result/{}/{}/{}".format(args.dataset_name, args.model, proportion_folder_name)
